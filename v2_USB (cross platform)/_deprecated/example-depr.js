...
/**
 * example.js — Interactive CLI example for the USB relay board
 *
 * Usage:
 *   node example.js        # auto-connects to the first found board
 *
 * Platform support:
...
const { UsbRelay, findDevices, IS_WINDOWS, IS_MAC, IS_LINUX } = require('./usbrelay');
...
async function main() {
  ...
  if (IS_WINDOWS) {
    console.log('Windows mode: using write() for HID communication.');
    console.log('If the board is not found, make sure you installed');
    console.log('libusb-win32 via Zadig and ran: npm install node-hid --driver=libusb\n');
  } else if (IS_MAC) {
    console.log('macOS mode: using getFeatureReport / sendFeatureReport.');
    console.log('If you get a permission error, try running with: sudo node example.js\n');
  } else {
    console.log('Linux mode: using getFeatureReport / sendFeatureReport.');
    console.log('If you get a permission error, add a udev rule or run with sudo.\n');
  }
